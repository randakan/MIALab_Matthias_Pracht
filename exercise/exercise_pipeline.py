...
def collect_image_paths(data_dir):
    ...
    crawler_ = futil.FileSystemDataCrawler(root_dir=data_dir,file_keys=image_keys,file_path_generator=MyFilePathGenerator,dir_filter=dir_filter)
...
def load_images(image_paths):
    ...
    image_dict = {
        structure.BrainImageTypes.T1w: sitk.ReadImage(image_paths.get(structure.BrainImageTypes.T1w),outputPixelType=sitk.sitkFloat32),
        structure.BrainImageTypes.GroundTruth: sitk.ReadImage(image_paths.get(structure.BrainImageTypes.GroundTruth),outputPixelType=sitk.sitkUInt8)
    }
...
def register_images(image_dict, atlas_img):
    ...
    registered_t1 = registration.execute(atlas_img,registration_params)
...
def preprocess_filter_rescale_t1(image_dict, new_min_val, new_max_val):
    ...
    minmax_rescaled_img = filter_.execute(image_dict.get(structure.BrainImageTypes.T1w),filter_params)
...
def extract_feature_median_t1(image_dict):
    ...
    median_img = filter_.execute(image_dict.get(structure.BrainImageTypes.T1w))
